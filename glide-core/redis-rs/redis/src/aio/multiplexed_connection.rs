...
pin_project! {
    struct PipelineSink<T> {
        ...
        // Epoch millis of last successful response received. Used by validate_all_user_connections
        // to detect stuck connections before the write timeout fires.
        last_response_time: Arc<AtomicU64>,
    }
            ...
            .poll_flush(cx)
        {
            ...
            Poll::Pending => {
                ...
                if now.duration_since(*start) >= *this.write_timeout {
                    ...
                    // Drain all InFlight entries with errors and mark connection closed.
                    log_error(
                        "PipelineSink",
                        format!(
                ...
                // Not timed out yet — schedule a wakeup so we re-check.
                // The underlying stream's poll_flush already registered a waker,
                // but we also need to wake up to check the timeout.
    ...
    SinkItem: Send + 'static,
{
        ...
        T::Error: ::std::fmt::Debug,
    {
        ...
        // When full, send_recv returns FatalSendError rather than blocking indefinitely.
