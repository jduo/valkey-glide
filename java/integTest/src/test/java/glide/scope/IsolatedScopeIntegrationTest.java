...
public class IsolatedScopeIntegrationTest {
    ...
    public void testScopeIsolation() throws Exception {
        ...
        // Note: The exact response depends on how redis-rs returns nil for failed EXEC
    ...
    public void testConcurrentScopeAcquisition() throws Exception {
        ...
        java.util.concurrent.ConcurrentHashMap<Long, Boolean> scopeIds = new java.util.concurrent.ConcurrentHashMap<>();
        ...
        for (int t = 0; t < numThreads; t++) {
            ...
            new Thread(() -> {
                try {
                    ...
                    // Record scope_id — all should be unique (each thread gets its own connection)
        ...
        // Note: scope_ids may be reused (LIFO pooling), so uniqueness isn't guaranteed
        // across sequential borrows. What matters is all threads succeeded without
        // data corruption — each thread's SET/GET returned consistent results.
